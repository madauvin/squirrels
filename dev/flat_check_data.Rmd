...
#' @param data data.frame portant sur les ecureuils
#' @return logical
#' 
#' @export
...
#' filter_squirrels_data_based_on_integrity
#' 
#' s'assure que les données vérifient toutes les conditions pour etre utilisée par l'utilisateur final du package et retourne un dataframe en fonction de son intégrité
#' 
#' @return df_valid_data dataframe, données pouvant etre utlilisées
#' @return df_invalid_data dataframe, données ne pouvant PAS etre utlilisées
#' @return df_missing_data  dataframe, données dont on ne peut vérifier l'intégrité.
#' @importFrom 
...
check_columns <-  check_presence_of_required_columns(dataframe = data)
 ...
 coordinates_are_ok <-  check_coordinates(
   latitude = data$lat, 
   longitude = data$long)
...
test_that("filter_squirrels_data_based_on_integrity works", {
  expect_true(
    inherits(filter_squirrels_data_based_on_integrity, "function")) })
  ...
  expect_equal(nrow(data_integrity$df_valid_data), 393)
